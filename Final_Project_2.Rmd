...
# Train ANN - Please note, we were unable to train anything more complex than 1 hidden layer neurons without significantly reducing the training set size or increasing training parameters (e.g., reps) which caused training to take an impracticable amount of time
ANN <- neuralnet(is_canceled ~ ., data = hotels_numeric_train, hidden = 1)
summary(ANN)
```
...
model_results <- neuralnet::compute(ANN, hotels_numeric_test)
...
Finally, we created the ANN and KNN models. We found the KNN model to achieve an accuracy of 71% and the ANN model to achieve an accuracy of 66%, which is slightly lower than other models. It is likely that with more computing power, a more accurate ANN model could be created to bring it closer in line to the other top performing models. 

In conclusion, our data set and the factors included proved to be significant in determining hotel cancellations and in being good predictors of hotel cancellations, as shown by our models. Therefore our business question is answered, that indeed hotel cancellations can be predicted with reasonable (>69%) accuracy based on fields/factors collected on the average customer. If we were to recommend a solution to a hotelier, we would suggest a decision tree model, since it was the most accurate of the models that we tested and it also has the benefit of being interpretable. This is particularly important in this case, since there may be actions which could be taken, based on the probable reasons that a customer would cancel their reservation, to act prior to a cancellation in order to convince a customer to maintain their business. 
